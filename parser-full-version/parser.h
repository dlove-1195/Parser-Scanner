...
void executor_stmt_seq(stmt_seq_node *prog, id_list_node *start);
void delete_stmt_seq(stmt_seq_node *prog);

void parser_decl(decl_node *decl);
void printer_decl(decl_node *prog, int tabbing);
...
char fileName[20];
FILE *input;
decl_func_node *startFunc = NULL;		// pointers for functions
decl_func_node *endFunc = NULL;
frame_node *FP = NULL;
frame_node *SP = NULL;
...
void executor_program(program_node *prog, char* dataFileName) {
	...
	if((*prog).ssn != NULL){
		executor_stmt_seq((*prog).ssn, startNode);
	}
}
...
void printer_decl_seq(decl_seq_node *prog, int tabbing) {
	if (prog != NULL){
		printer_decl((*prog).decl, tabbing);
...
void parser_stmt_seq(stmt_seq_node *ssn) {
	...
	if(currentToken() != END && currentToken() != ENDIF && currentToken() != ENDWHILE && currentToken() != ELSE && currentToken() != ENDFUNC){
...
void executor_stmt_seq(stmt_seq_node *prog, id_list_node *start) {
	executor_stmt((*prog).stmt, start);
	if((*prog).stmtSeq != NULL){
		executor_stmt_seq((*prog).stmtSeq, start);
	}
}
...
void parser_decl(decl_node *decl) {
	if(currentToken() == INT){
		...
		(*decl).decl_func = (decl_func_node*)malloc(sizeof(decl_func_node));
		parser_decl_func((*decl).decl_func);
	}
	else{
		printf("Expected INT or ID token in the declaration\n");
		exit(0);
	}
}
...
void printer_decl(decl_node *prog, int tabbing) {
	...
	else if(prog->decl_func != NULL){
		printer_decl_func(prog->decl_func, tabbing);
	}
}
...
void executor_decl(decl_node *prog) {
	if(prog->idList != NULL){
		...
		executor_decl_func((*prog).decl_func);
	}
}
...
void delete_decl(decl_node *prog) {
	...
	if(prog->decl_func != NULL){
		delete_decl_func((*prog).decl_func);
	}
	free((*prog).idList);
	free(prog->decl_func);
}
...
void parser_stmt(stmt_node *prog) {
	if(currentToken() == ID){
		...
		(*prog).funcNode = (func_node*)malloc(sizeof(func_node));
		parser_func((*prog).funcNode);
		(*prog).identity = 6;
	}
	else{
		printf("ERROR: Expected ID/IF/WHILE/INPUT/OUTPUT/BEGIN token\n");
		exit(0);
	}
}
...
void printer_stmt(stmt_node *prog, int tabbing) {
	...
	else if(prog->identity == 6){
		for (i = 0; i < tabbing; i++){
			printf("\t");
		}
		printer_func(prog->funcNode);
	}
}

void executor_stmt(stmt_node *prog, id_list_node *start) {
	if((*prog).identity == 1){
		executor_assign((*prog).assignNode, start);
	}
	else if((*prog).identity == 2){
		executor_if((*prog).ifNode, start);
	}
	else if((*prog).identity == 3){
		executor_loop((*prog).loopNode, start);
	}
	else if((*prog).identity == 4){
		executor_in((*prog).inNode, start);
	}
	else if((*prog).identity == 5){
		executor_out((*prog).outNode, start);
	}
	else if(prog->identity == 6){
		executor_func(prog->funcNode, start);
	}
}
...
void executor_term(term_node *prog, id_list_node *start){
	executor_factor((*prog).factor, start);
	if((*prog).term == NULL){
...
void executor_if(if_node *prog, id_list_node *start){
	executor_cond(prog->cond, start);
	if(prog->cond->value){
		executor_stmt_seq(prog->stmt_seq, start);
	}
	else{
		if(prog->stmt_seq_seq != NULL){
			executor_stmt_seq(prog->stmt_seq_seq, start);
		}
	}
}
void delete_if(if_node *prog){
...
void executor_cmpr(cmpr_node *prog, id_list_node *start){
	executor_expr(prog->expr, start);
	executor_expr(prog->expr_expr, start);
	if(prog->identity == 1){
...
void parser_decl_func(decl_func_node *prog){
	...
	nextToken();
	(*prog).id_list = (id_list_node*)malloc(sizeof(id_list_node));
	parser_id_list((*prog).id_list);
	if(currentToken() != RPAREN){
		printf("ERROR: Expected ) token\n");
		exit(0);
	}
	...
	if(currentToken() != ENDFUNC){
		printf("ERROR: Expected endfunc token\n");
		exit(0);
	}
	nextToken();
	if(currentToken() != SEMICOLON){
...
void executor_decl_func(decl_func_node *prog){
	...
	decl_func_node *traverse = startFunc;
	while(traverse != NULL){
		if(strcmp(traverse->id, prog->id) == 0){
			printf("ERROR: Expected unique function name\n");
			exit(0);
		}
		traverse = traverse->next;
	}
...
void delete_decl_func(decl_func_node *prog){
	if(prog->id_list != NULL){
		delete_id_list(prog->id_list);
	}
	if(prog->stmt_seq != NULL){
		delete_stmt_seq(prog->stmt_seq);
	}
	free(prog->id_list);
	free(prog->stmt_seq);
...
/* func functions */
void parser_func(func_node *prog){
	nextToken();	// consume BEGIN token
	if(currentToken() != ID){
		printf("Expected ID token after the BEGIN token of function\n");
		exit(0);
	}
	getID(prog->id);
	nextToken();	// consume the ID token
...
void executor_func(func_node *prog, id_list_node * start){
	...
	while(1){
		if(traverse == NULL){
			printf("ERROR: Expected declaration of function %s before using it\n", prog->id);
			exit(0);
		}
		if(strcmp((*traverse).id,(*prog).id) == 0){
			break;
	...
	// find values of needed paramters
	id_list_node *tr_para = prog->id_list;
	while(tr_para != NULL){
		id_list_node *tr_value = start;
		while(1){
			if(tr_value == NULL){
				printf("ERROR: Expected value for paramter %s \n", tr_para->id);
				exit(0);
			}
			if(strcmp((*tr_para).id,(*tr_value).id) == 0){
				tr_para->value = tr_value->value;
				break;
			}
			tr_value = (*tr_value).next;
		}
		tr_para = tr_para->idList;
	}

	// add stack frame
	frame_node *currentFrame = (frame_node*)malloc(sizeof(frame_node));
	if(FP == NULL){
		FP = currentFrame;
		SP = currentFrame;
	}
		...
		frame_node *pre = SP;
		SP->next = currentFrame;
		SP = currentFrame;
		SP->previous = pre;
	}

	// copy values of arguments to formal paramters on stack
	id_list_node *tr_formal = currentFrame->id_list;
	tr_para = prog->id_list;
	id_list_node *para_name = traverse->id_list;
	while(1){
		if(tr_para == NULL){
			break;
		}
		id_list_node *newNode = (id_list_node*)malloc(sizeof(id_list_node));
		if(tr_formal == NULL){
			currentFrame->id_list = newNode;
			tr_formal = newNode;
		}
		else{
			tr_formal->next = newNode;
		}
		strcpy(newNode->id, para_name->id);
		newNode->value = tr_para->value;
		newNode->initialized = 1;
		tr_para = tr_para->idList;
		para_name = para_name->idList;
	}
	executor_frame(currentFrame, traverse->stmt_seq);
	
	// copy values from stack back to function
	tr_formal = currentFrame->id_list;
	tr_para = prog->id_list;
	while(tr_formal != NULL){
		tr_para->value = tr_formal->value;
		tr_para = tr_para->idList;
		tr_formal = tr_formal->next;
	}
...
void executor_frame(frame_node *prog, stmt_seq_node *stmt_seq){
	executor_stmt_seq(stmt_seq, prog->id_list);
